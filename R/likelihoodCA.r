...
#'@description R function to compute the likelihood of detecting contingent absences within distributional dataset
#'@usage likelihoodCA(occData=occData,envData=envData,longlat=T,coords=NULL,nf=NULL)
...
#'@details This function campute the likelihood of detecting contingent absences within distributional dataset. It's based on the idea that absences data that are environmentally close but geographically distant to presences data are more likely to be contingent absences.  
...
likelihoodCA<-function(occData,coords=NULL, envData, longlat=TRUE,nf=5){
  ...
  if (longlat==TRUE){distGeo<-NULL
                     ...
                     geoWeight<-c(1-((distGeo-min(distGeo))/(max(distGeo)-min(distGeo))))}
  ...
  if(longlat==FALSE){distGeo<-pdist(coordinates(presence),coordinates(absence))
                     ...
                     geoWeight<-1-((distGeo-min(distGeo))/(max(distGeo)-min(distGeo)))}
  ...
  envWeight<-(envWeight-min(envWeight))/(max(envWeight)-min(envWeight)) 
  
  absence@data$envWeight<-envWeight
  presence@data$envWeight<-rep(1,dim(presence)[1])
  occRes<-spRbind(presence,absence)
  occRes$LCA<-1-((occRes$envWeight+occRes$geoWeight)/2)
  occRes<-occRes[,"LCA"]
  return(occRes)
