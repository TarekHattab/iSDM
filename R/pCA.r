#' R function to compute the probability of detecting contingent absences within a network of presence-absence data
#'@description R function to compute the probability of detecting contingent absences
#'@usage pCA(occData,envData, longlat=TRUE,nf=5,occNative=NULL,envNative=NULL)
...
#'@param envData An abject containing your explanatory variables. Either a SpatialPointsDataFrame or SpatialGridDataFrame as defined in package sp or a RasterStack or RasterBrick as defined in package raster. Note that this object can contain a mixture of variables type (quantitative, factor and ordered).
...
#'
#' # Calculat  the probability of detecting contingent absences
#'probability<-pCA(occData=occData,envData=envData[[c(1,5,6)]],longlat=TRUE)
#'
#' # Display the results
...
#'return(zColors)}
#'
#'points(probability,pch=21, col=1,bg=scatterCol(probability@data[,"PCA"]),cex=1)
#'
...
pCA<-function(occData, envData, longlat=TRUE,nf=5,occNative=NULL,envNative=NULL){
  ...
  if (!all(c(is.null(occNative),is.null(envNative)))) {if (!(class(envNative)%in% c("SpatialPointsDataFrame","SpatialGridDataFrame","SpatialPixelsDataFrame","RasterStack","RasterBrick"))) {stop("The occNative object must be either a SpatialPointsDataFrame or a SpatialGridDataFrame or a RasterStack or a RasterBrick")}} 
  ...
  if (!all(c(is.null(occNative),is.null(envNative)))) {if(class(envData) %in% c("SpatialGridDataFrame,SpatialPointsDataFrame","SpatialPixelsDataFrame"))
  {occEnv1<-over(occData,envData)
   occEnv1<-cbind(SP=occData1@data[,1],occEnv1)
   occEnv2<-over(occNative,envNative)
  ...
  {occEnv1<-extract(y=occData,x=envData)
   occEnv1<-cbind(SP=occData@data[,1],occEnv1)
   occEnv2<-extract(y=occNative,x=envNative)
   occEnv2<-cbind(SP=rep(1,nrow(occEnv2)),occEnv2)
   occEnv<-rbind(occEnv1,occEnv2)}}
  ...
  if(any(sapply(occEnv,is.factor)[-1])|any(sapply(occEnv,is.ordered)[-1])){
    ordi<-dudi.mix(occEnv[,-1],scannf=F,nf=nf)
  ...
  occRes$PCA<-(occRes$envWeight+occRes$geoWeight)/2
  occRes<-occRes[,"PCA"]
  return(occRes)
