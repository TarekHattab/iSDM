...
#'@param nf Numeric of ordination axes retained in the analysis, this function manages currently only 2 or 3 dimentions
...
#'@return Returns a list containing 4 objects GeoSamples (The geographical coordinates of the pixels corresponding to the systemic sampling design).EnvSamples (The coordinates in the reduced environmental space of of the pixels corresponding to the systemic sampling design).EnvGrid (The coordinates of the grid in the reduced environmental).cellsize (he sizes of the grid cells in the different dimensions)
...
EnvSysSample<-function(envData,nExpect=100,plot=T,saveShape=T,nf=3,lowerLim=0.00005,upperLim=0.99995){
  ...
  if(plot==T & nf==3){open3d()
                      plot3d(x[,1], x[,2], x[,3], r = 0.2,xlab="Axis 1",ylab="Axis 2",zlab="Axis 3")
                      plot3d(output[[2]][,1],output[[2]][,2],output[[2]][,3], col=2, type ="s",r=.1,add=T)
                      rgl.triangles(x[conv,1],x[conv,2],x[conv,3],col=2,alpha=0.2)
                      moncube<-cube3d(col = "red")
                      moncube$vb[1,] <- moncube$vb[1,]*output[[4]][1]
                      moncube$vb[2,] <- moncube$vb[2,]*output[[4]][2]
                      moncube$vb[3,] <- moncube$vb[3,]*output[[4]][3]
                      for(i in 1:dim(output[[3]])[1]){wire3d( translate3d( moncube, output[[3]][i,1], output[[3]][i,2],output[[3]][i,3]))}}
