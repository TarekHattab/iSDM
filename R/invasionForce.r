...
#'@usage  invasionForce(occData,coords,a,envData,binary=T,longlat=NULL)
...
#'@param binary logical TRUE if the occData object contains presences/absences (0/1) data and FALSE if occData contains only the geographical coordinates of presence
...
#'propagule<-invasionForce(occData,envData=envData,a=a,binary=T,longlat=TRUE)
#'plot(propagule,main=paste("a = ",a))
#'plot(occData,col=ifelse(occData@data[,1]==1,1,0),add=T,cex=0.3)}

invasionForce<-function(occData,coords=NULL,a=NULL,envData,binary=T,longlat=NULL){
